...
export const QuestionCategoriesPage = () => {
  ...
  useEffect(() => {
    ...
    void Promise.all([getQuestionCategories(), getSchoolGroups()])
      .then(([foundCategories, foundGroups]) => {
        ...
        setGroup(
          foundGroups.find((item) => item.type === "department")?.id ?? 0,
        );
      })
      .catch((reason) => {
  ...
  const save = async (event: React.FormEvent) => {
    ...
    try {
      if (editing) {
        const updated = await updateQuestionCategory(editing.id, {
          name,
          group_id: group,
  ...
  const toggleActive = async (item: QuestionCategory) => {
    ...
    try {
      const updated = await updateQuestionCategory(item.id, {
        name: item.name,
      ...
      setError(
        reason instanceof Error ? reason.message : `${action}できませんでした`,
      );
    }
  };
    ...
    <main className="min-h-dvh bg-transparent p-6 text-slate-900">
      <div className="mx-auto max-w-3xl">
        ...
        <p className="mt-1 text-sm text-slate-600">
          カテゴリ名と担当部署を編集できます。不要なカテゴリは過去の質問を残したまま無効化します。
        </p>
        {error && (
        ...
        <form onSubmit={save} className="my-6 flex flex-wrap gap-3">
          <input
            ...
            placeholder="進路・奨学金"
            className="min-w-60 flex-1 rounded border border-slate-300 bg-white p-3 text-slate-900"
          />
          <select
            value={group}
            onChange={(event) => setGroup(Number(event.target.value))}
            className="rounded border border-slate-300 bg-white p-3 text-slate-900"
          >
          ...
          <button
            ...
            className="rounded bg-sky-600 px-4 text-white disabled:opacity-40"
          >
            {editing ? "変更を保存" : "追加"}
          </button>
          {editing && (
            <button
              ...
              className="rounded border border-slate-300 px-4"
            >
              キャンセル
            </button>
          )}
        ...
        <ul className="space-y-3">
          {categories.map((item) => (
            <li
              key={item.id}
              className={`flex flex-wrap items-center gap-3 rounded-xl border bg-white p-4 ${item.is_active ? "border-sky-100" : "border-slate-200 opacity-65"}`}
            >
              ...
              <button
                ...
                className="rounded border border-sky-300 px-3 py-2 text-sm text-sky-700"
              >
                編集
              </button>
              <button
