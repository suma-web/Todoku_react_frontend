    ...
    <main className="min-h-dvh bg-transparent p-6 text-slate-900">
      <div className="mx-auto max-w-2xl">
        ...
        <form onSubmit={submit} className="my-6 flex gap-3">
          <input value={name} onChange={(event) => setName(event.target.value)} placeholder="進路・奨学金" className="flex-1 rounded bg-white border border-slate-300 text-slate-900 p-3" />
          <select value={group} onChange={(event) => setGroup(Number(event.target.value))} className="rounded bg-white border border-slate-300 text-slate-900 p-3">
            {groups.filter((item) => item.type === "department").map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}
          </select>
          <button disabled={!name.trim() || !group} className="rounded bg-sky-600 text-white px-4 disabled:opacity-40">追加</button>
        </form>
        <ul>{categories.map((item) => <li key={item.id} className="border-b border-slate-200 py-3">{item.name} → {item.group_name}</li>)}</ul>
      </div>
    </main>
  );
};
