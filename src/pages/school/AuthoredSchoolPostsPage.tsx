...
export const AuthoredSchoolPostsPage = () => {
  ...
  return <main className="p-6"><div className="mx-auto max-w-3xl">
    ...
    <div className="mt-5 space-y-4">{posts.map((post) => { const warning = schoolPostAgeWarning(post.created_at, post.expires_at); const expired = isExpiredSchoolPost(post.expires_at); return <article key={post.id} className="rounded-2xl border border-sky-100 bg-white p-5 shadow-sm">
      ...
      {expired && <p className="mt-2 text-sm font-bold text-red-700">有効期限切れ（通常ユーザーには表示されません）</p>}
