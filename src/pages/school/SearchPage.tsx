...
export const SearchPage = () => {
  ...
  return <main className="min-h-dvh bg-transparent p-5 text-slate-900"><div className="mx-auto max-w-3xl">
    ...
    <div className="space-y-3">{items.map((item) => {
      ...
      return <button key={`${item.type}-${item.id}`} onClick={() => open(item)} disabled={item.type === "contact"} className="block w-full rounded-2xl border border-sky-100 bg-white p-4 text-left shadow-sm disabled:opacity-100">
