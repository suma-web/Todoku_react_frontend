...
export const PostStatusPage = () => {
  ...
  useEffect(() => {
    ...
    void getSchoolPostStatus(Number(id)).then((result) => { if (active) setStatus(result); })
      .catch((reason) => { if (active) setError(reason instanceof Error ? reason.message : "既読状況を取得できませんでした"); });
    return () => { active = false; };
  }, [id]);

  return <main className="p-6"><div className="mx-auto max-w-3xl">
    <h1 className="text-2xl font-bold">既読状況</h1>
    ...
    {status && <>
      <div className="my-6 grid grid-cols-3 gap-3">{[["対象者", status.target_count], ["既読", status.read_count], ["未読", status.unread_count]].map(([label, value]) => <div key={label} className="rounded-xl border border-sky-100 bg-white p-4 shadow-sm"><strong className="block text-2xl">{value}</strong>{label}</div>)}</div>
      <div className="grid gap-4 md:grid-cols-2">
        <UserList title="既読" users={status.read_users} empty="既読にした人はいません。" />
