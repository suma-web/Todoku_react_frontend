...
export const TimelinePage = () => {
  ...
  return <main className="min-h-dvh bg-transparent p-5 text-slate-900"><div className="mx-auto max-w-3xl">
    ...
    {visible.map((post) => { const warning = schoolPostAgeWarning(post.created_at, post.expires_at); return <Link key={post.id} to={`/school-posts/${post.id}`} className="mb-4 block rounded-2xl border border-sky-100 bg-white p-5 shadow-sm transition hover:border-sky-300 hover:shadow-md">
      ...
      {warning && <p className="mt-3 rounded-lg bg-amber-50 px-3 py-2 text-sm text-amber-800">{warning}</p>}
    </Link>; })}
  </div></main>;
};
