...
export type QuestionCategory = { id:number; name:string; group_id:number; group_name:string; is_active:boolean };
...
export const updateQuestionCategory=(id:number,input:{name:string;group_id:number;is_active:boolean})=>read<QuestionCategory>(fetch(`${API_BASE_URL}/api/question-categories/${id}`,options("PATCH",input)));
